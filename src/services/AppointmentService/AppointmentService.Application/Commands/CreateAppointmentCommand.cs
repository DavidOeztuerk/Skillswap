    ...
    : ICommand<CreateAppointmentResponse>, IAuditableCommand, ICacheInvalidatingCommand
{
    ...
    // FIXED: Invalidate cache for BOTH users (organizer and participant)
    // Placeholders {UserId} and {ParticipantUserId} will be replaced by CacheInvalidationBehavior
    public string[] InvalidationPatterns => new[]
    {
        "user-appointments:{UserId}:*",           // Invalidate organizer's appointments
        "user-appointments:{ParticipantUserId}:*",  // Invalidate participant's appointments
        "appointment-details:*"                    // Invalidate all appointment details
    };
}
