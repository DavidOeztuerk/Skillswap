...
public class NotificationHub : Hub
{
    ...
    /// <summary>
    /// Send new appointment notification to user
    /// </summary>
    public static async Task SendNewAppointmentToUser(
        IHubContext<NotificationHub> hubContext,
        string userId,
        object appointment)
    {
        await hubContext.Clients.Group($"user-{userId}").SendAsync("NewAppointment", appointment);
    }

    /// <summary>
    /// Send multiple appointments to user (for batch creation from match)
    /// </summary>
    public static async Task SendNewAppointmentsToUser(
        IHubContext<NotificationHub> hubContext,
        string userId,
        object appointments)
    {
        await hubContext.Clients.Group($"user-{userId}").SendAsync("NewAppointments", appointments);
    }

    /// <summary>
    /// Send appointment update to user
    /// </summary>
    public static async Task SendAppointmentUpdateToUser(
        IHubContext<NotificationHub> hubContext,
        string userId,
        object appointment)
    {
        await hubContext.Clients.Group($"user-{userId}").SendAsync("AppointmentUpdated", appointment);
    }
