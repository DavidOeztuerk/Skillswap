    ...
    : BaseCommandHandler<UpdateNotificationPreferencesCommand, UpdateNotificationPreferencesResponse>(logger)
{
  ...
  {
    var existingPreferences = await _unitOfWork.NotificationPreferences
        .GetByUserIdAsync(request.UserId, cancellationToken);

    var isNew = existingPreferences == null;

    // Create new or use existing preferences
    var preferences = existingPreferences ?? new NotificationPreferences
    {
    ...
    // Only call CreateAsync for new entities - existing entities are already tracked
    if (isNew)
    {
      await _unitOfWork.NotificationPreferences.CreateAsync(preferences, cancellationToken);
    }
    // For existing entities, EF Core change tracking will handle the update automatically
