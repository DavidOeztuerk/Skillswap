...
/// <summary>
/// Refactored UpdateSkillCommandHandler with improved separation of concerns
/// </summary>
public class UpdateSkillCommandHandlerRefactored(
    SkillDbContext dbContext,
    IDomainEventPublisher eventPublisher,
    ILogger<UpdateSkillCommandHandlerRefactored> logger)
    : BaseCommandHandler<UpdateSkillCommand, UpdateSkillResponse>(logger)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await UpdateCategoryCountsAsync(skill.SkillCategoryId, newCategoryId, cancellationToken);
    ...
    private async Task UpdateCategoryCountsAsync(string? oldCategoryId, string newCategoryId, CancellationToken cancellationToken)
    {
        if (!string.IsNullOrEmpty(oldCategoryId))
        {
            var oldCategory = await _dbContext.SkillCategories
                .FirstOrDefaultAsync(c => c.Id == oldCategoryId, cancellationToken);
            if (oldCategory != null)
            {
                oldCategory.SkillCount = Math.Max(0, oldCategory.SkillCount - 1);
                oldCategory.ActiveSkillCount = Math.Max(0, oldCategory.ActiveSkillCount - 1);
            }
        }
    ...
    private async Task UpdateCategoryActiveCountAsync(string? categoryId, bool isActive, CancellationToken cancellationToken)
    {
        ...
        if (category != null)
        {
            category.ActiveSkillCount += isActive ? 1 : -1;
            category.ActiveSkillCount = Math.Max(0, category.ActiveSkillCount);
        }
    }
