...
public class ListingRepository : IListingRepository
{
  ...
  /// ONLY returns listings that have an active boost (paid feature)
  /// Sorted by: Boost type > Rating > Popularity > Recency
  /// </summary>
      ...
      CancellationToken cancellationToken = default)
  {
    ...
    return await _dbContext.Listings
        ...
        .Where(l =>
            ...
            l.Skill.IsActive &&
            !l.Skill.IsDeleted &&
            // KRITISCH: Nur geboostete Listings mit aktivem Boost zeigen!
            l.IsBoosted &&
            l.BoostedUntil.HasValue &&
            l.BoostedUntil.Value > now)
        // Sortierung: Premium-Boosts zuerst, dann Rating, dann Popularität
        .OrderByDescending(l => l.IsTopListing)      // Top-Anzeige (höchster Preis)
        .ThenByDescending(l => l.IsInGallery)        // Galerie
        .ThenByDescending(l => l.IsHighlighted)      // Highlight
