...
/// </summary>
public class CompleteXingConnectCommandHandler : IRequestHandler<CompleteXingConnectCommand, ApiResponse<XingConnectionResponse>>
{
    private readonly IXingService _xingService;
    private readonly IUserXingConnectionRepository _repository;
    private readonly ITokenEncryptionService _encryptionService;
    private readonly IDistributedCache _cache;
    private readonly ILogger<CompleteXingConnectCommandHandler> _logger;

    public CompleteXingConnectCommandHandler(
    ...
    public async Task<ApiResponse<XingConnectionResponse>> Handle(
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                return ApiResponse<XingConnectionResponse>.ErrorResult("Invalid state token");
            }

            // Verify request token matches
            if (requestToken != request.OAuthToken)
            {
                return ApiResponse<XingConnectionResponse>.ErrorResult("OAuth token mismatch");
            }
            ...
            if (existing != null)
            {
                return ApiResponse<XingConnectionResponse>.ErrorResult(
                    "Xing is already connected");
            }
            ...
            {
                return ApiResponse<XingConnectionResponse>.ErrorResult(
                    "Request token expired. Please try again.");
            }
            ...
            {
                _logger.LogWarning("Xing token exchange failed for user {UserId}: {Error}",
                    userId, tokenResult.Error);
                return ApiResponse<XingConnectionResponse>.ErrorResult(
                    tokenResult.Error ?? "Failed to exchange verifier for tokens");
            }
            ...
            _logger.LogInformation("Xing connection created for user {UserId} with Xing ID {XingId}",
                userId, tokenResult.XingId);

            return ApiResponse<XingConnectionResponse>.SuccessResult(
                MapToResponse(connection),
                "Xing connected successfully");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error completing Xing connection");
            return ApiResponse<XingConnectionResponse>.ErrorResult("Failed to complete Xing connection");
        }
    }
