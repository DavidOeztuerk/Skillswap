...
import { processSessionPayment } from '../store/sessionsThunks';
import type { RootState } from '../../../core/store/store';
import type { ProcessSessionPaymentRequest } from '../services/sessionService';
...
const PaymentConfirmation: React.FC<PaymentConfirmationProps> = ({
  ...
  const platformFeePercent = 10; // Default 10% platform fee
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogContent dividers>
        <Stack spacing={3} sx={{ mt: 1 }}>
          {error ? (
            <Alert severity="error">{error.message ?? 'Failed to process payment'}</Alert>
          ) : null}
