...
const SessionHistoryTable: React.FC<SessionHistoryTableProps> = ({ onViewDetails, onExport }) => {
  ...
  const uniqueStatuses = [...new Set(sessionHistory.map((s: SessionStatus) => s.status))];
    ...
    <Box>
            ...
            {paginatedSessions.map((session: SessionStatus) => (
              <TableRow key={session.sessionAppointmentId} hover>
                <TableCell sx={{ fontFamily: 'monospace', fontSize: '0.75rem' }}>
                  {session.sessionAppointmentId.slice(0, 16)}...
                </TableCell>
                <TableCell>
                  <Chip
      ...
      <Box sx={{ mt: 2, p: 1, bgcolor: 'grey.50', borderRadius: 1 }}>
        <Typography variant="caption" color="textSecondary">
          ...
          {Math.min((page + 1) * rowsPerPage, filteredSessions.length)} of {filteredSessions.length}{' '}
          sessions
        </Typography>
      </Box>
    </Box>
