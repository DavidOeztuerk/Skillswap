...
// Constants
const APPOINTMENT_ID_REQUIRED = 'Appointment ID is required';
...
const sessionService = {
  ...
  async startSession(appointmentId: string): Promise<ApiResponse<SessionStatusResponse>> {
    if (!appointmentId.trim()) throw new Error(APPOINTMENT_ID_REQUIRED);

    return apiClient.post<SessionStatusResponse>(
