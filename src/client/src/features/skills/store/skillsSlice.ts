...
import { initialSkillsState, skillsAdapter } from './skillsAdapter+State';
import {
  fetchUserSkills,
  fetchSkillById,
  createSkill,
...
const skillsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAllSkills.rejected, (state, action) => {
      console.error('❌ [skillsSlice] fetchAllSkills.rejected:', action.payload);
      state.isLoading = false;
      state.errorMessage =
    ...
    builder.addCase(fetchUserSkills.rejected, (state, action) => {
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to fetch user skills';
    });
    ...
    builder.addCase(fetchSkillById.rejected, (state, action) => {
      console.error('❌ [skillsSlice] fetchSkillById.rejected:', action.payload);
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to fetch skill';
    });
    ...
    builder.addCase(createSkill.fulfilled, (state, action) => {
      ...
      // Add to both collections (prepend so newest appears first)
      if (!state.allSkillIds.includes(action.payload.id)) {
        state.allSkillIds.unshift(action.payload.id);
      }
      if (!state.userSkillIds.includes(action.payload.id)) {
        state.userSkillIds.unshift(action.payload.id);
      }
    });
    builder.addCase(createSkill.rejected, (state, action) => {
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to create skill';
    });
    ...
    builder.addCase(updateSkill.rejected, (state, action) => {
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to update skill';
    });
    ...
    builder.addCase(deleteSkill.rejected, (state, action) => {
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to delete skill';
    });
    ...
    builder.addCase(fetchFavoriteSkills.rejected, (state, action) => {
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to fetch favorite skills';
    });
    ...
    builder.addCase(toggleFavoriteSkill.rejected, (state, action) => {
      state.isLoading = false;
      state.errorMessage =
        action.payload?.message ?? action.error.message ?? 'Failed to toggle favorite skill';
    });
  },
});
