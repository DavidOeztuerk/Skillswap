...
import type { GetUserSkillResponse } from '../types/SkillResponses';
...
const FavoriteSkillsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFavortiteSkills = async (): Promise<void> => {
      await loadFavoriteSkills();
    };

    fetchFavortiteSkills().catch(() => {});
  }, [loadFavoriteSkills]);
              ...
              <CardContent sx={{ flexGrow: 1 }}>
                ...
                {skill.tags.length > 0 && (
                  <Stack direction="row" spacing={0.5} sx={{ mt: 2 }} flexWrap="wrap">
                    ...
                    {skill.tags.length > 3 && (
                      <Chip label={`+${skill.tags.length - 3}`} size="small" sx={{ mb: 0.5 }} />
                    )}
                  </Stack>
                )}
