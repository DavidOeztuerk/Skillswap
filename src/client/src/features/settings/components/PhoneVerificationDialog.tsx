...
const PhoneVerificationDialog: React.FC<PhoneVerificationDialogProps> = ({
  ...
  const handlePaste = (e: React.ClipboardEvent): void => {
    e.preventDefault();
    const pastedData = e.clipboardData.getData('text').replaceAll(/\D/g, '').slice(0, 6);
          ...
          <Box>
            <Stack spacing={3}>
              ...
              <Box sx={{ textAlign: 'center' }}>
                ...
                {(() => {
                  if (isLoading('resendPhoneCode')) {
                    return <CircularProgress size={20} />;
                  }
                  if (resendCooldown > 0) {
                    return `Erneut senden in ${resendCooldown}s`;
                  }
              ...
              <Controller name="code" control={codeForm.control} render={() => <></>} />
            </Stack>
          </Box>
        );
