...
const OVERLAY_BACKGROUND = 'rgba(0, 0, 0, 0.3)';

// ============================================================================
// PERFORMANCE: Extract sx objects as constants to prevent recreation
// ============================================================================

const layoutControlsSx: SxProps<Theme> = {
  position: 'absolute',
  top: 16,
  right: 16,
  display: 'flex',
...
const mainVideoBaseSx: SxProps<Theme> = {
  flex: 1,
  position: 'relative',
...
const sideParticipantsContainerSx: SxProps<Theme> = {
  width: 200,
  display: 'flex',
...
const screenShareParticipantSx: SxProps<Theme> = {
  height: 100,
  position: 'relative',
  backgroundColor: OVERLAY_BACKGROUND,
  borderRadius: 1,
...
const VideoLayout: React.FC<VideoLayoutProps> = ({
    ...
    switch (activeLayoutMode) {
      case 'spotlight': {
        ...
        return {
          ...
          sideParticipants:
            speaker === undefined ? participants : participants.filter((p) => p.id !== speaker.id),
        };
      }
  ...
  const renderLayoutControls = useCallback(
    () => (
      <Paper elevation={4} sx={layoutControlsSx}>
    ...
    () => (
      <Box sx={spotlightContainerSx}>
        ...
        {sideParticipants.length > 0 && (
          <Box sx={sideParticipantsContainerSx}>
  ...
  const renderPipLayout = useCallback(
    () => (
      <Box sx={fullSizeContainerSx}>
        ...
        >
          {mainParticipant ? <ParticipantLabel name={mainParticipant.name} /> : null}
        </Box>
      ...
      <Box sx={spotlightContainerSx}>
        ...
        <Box sx={screenShareSidebarSx}>
          {participants.slice(0, 6).map((participant) => (
            <Box key={participant.id} sx={screenShareParticipantSx}>
              <ParticipantLabel name={participant.name} isLocal={participant.isLocal} small />
            </Box>
          ))}
        </Box>
  ...
  if (children) {
    return (
      <Box sx={fullSizeContainerSx}>
        {showLayoutControls ? renderLayoutControls() : null}
        {children({
