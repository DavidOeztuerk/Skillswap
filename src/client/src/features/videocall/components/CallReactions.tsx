...
const DEFAULT_RECEIVED_REACTIONS: ReceivedReaction[] = [];
...
> = {
  thumbsUp: {
    ...
    emoji: '👍',
    label: 'Daumen hoch',
    color: featureColors.reactions.like,
  },
  heart: {
    icon: <HeartIcon />,
    emoji: '❤️',
    label: 'Herz',
    color: featureColors.reactions.love,
  },
  applause: {
    icon: <CelebrationIcon />,
    emoji: '👏',
    label: 'Applaus',
    color: featureColors.reactions.surprised,
  },
  laugh: {
    icon: <LaughIcon />,
    emoji: '😄',
    label: 'Lachen',
    color: featureColors.reactions.laugh,
  },
  wow: {
    icon: <StarIcon />,
    emoji: '🌟',
    label: 'Wow',
    color: featureColors.reactions.celebrate,
  },
};

const CallReactions: React.FC<CallReactionsProps> = ({
  onSendReaction,
  receivedReactions = DEFAULT_RECEIVED_REACTIONS,
  disabled = false,
  ...
  const processedReactionIdsRef = useRef(new Set<string>());
  ...
  // Handle new received reactions - use ref to track processed IDs without triggering re-renders
  useEffect(() => {
    const processedIds = processedReactionIdsRef.current;
