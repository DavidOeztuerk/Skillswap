...
import type { AdminEntityState } from './adminAdapter+State';
...
// Base selectors
export const selectAdminState = (state: RootState): AdminEntityState => state.admin;
export const selectAdminLoading = (state: RootState): boolean => state.admin.isLoading;
export const selectAdminError = (state: RootState): string | undefined => state.admin.errorMessage;
...
export const selectIsLoadingUsers = (state: RootState): boolean => state.admin.isLoadingUsers;
export const selectIsLoadingSkills = (state: RootState): boolean => state.admin.isLoadingSkills;
export const selectIsLoadingAppointments = (state: RootState): boolean =>
  state.admin.isLoadingAppointments;
export const selectIsLoadingMatches = (state: RootState): boolean => state.admin.isLoadingMatches;
export const selectIsLoadingAnalytics = (state: RootState): boolean =>
  state.admin.isLoadingAnalytics;
export const selectIsLoadingSystemHealth = (state: RootState): boolean =>
  state.admin.isLoadingSystemHealth;
export const selectIsLoadingAuditLogs = (state: RootState): boolean =>
  state.admin.isLoadingAuditLogs;
export const selectIsLoadingReports = (state: RootState): boolean => state.admin.isLoadingReports;
export const selectIsLoadingSettings = (state: RootState): boolean => state.admin.isLoadingSettings;

// Error state selectors
export const selectUserError = (state: RootState): string | undefined => state.admin.userError;
export const selectSkillError = (state: RootState): string | undefined => state.admin.skillError;
export const selectAppointmentError = (state: RootState): string | undefined =>
  state.admin.appointmentError;
export const selectMatchError = (state: RootState): string | undefined => state.admin.matchError;
export const selectAnalyticsError = (state: RootState): string | undefined =>
  state.admin.analyticsError;
export const selectSystemHealthError = (state: RootState): string | undefined =>
  state.admin.systemHealthError;
export const selectAuditLogError = (state: RootState): string | undefined =>
  state.admin.auditLogError;
export const selectReportError = (state: RootState): string | undefined => state.admin.reportError;
export const selectSettingsError = (state: RootState): string | undefined =>
...
export const selectUserFilters = createSelector([selectAdminFilters], (filters) => filters?.users);
...
export const selectAdminMatchFilters = createSelector(
  [selectAdminFilters],
...
export const selectFilteredUsers = createSelector(
  ...
  (users, filters) => {
    ...
    if (filters && typeof filters.searchTerm === 'string' && filters.searchTerm.trim()) {
...
export const selectFilteredAppointments = createSelector(
  ...
  (appointments, filters) => {
    ...
    // Filter by status - check if status is defined and not 'all'
    const statusFilter = filters?.status;
    if (statusFilter !== undefined && statusFilter !== 'all') {
      const targetStatus = statusFilter;
      filtered = filtered.filter((appointment) => appointment.status === targetStatus);
    }
    ...
    if (filters?.fromDate !== undefined && filters.fromDate.length > 0) {
      ...
      filtered = filtered.filter((appointment) => {
        const appointmentDate = new Date(appointment.createdAt).toDateString();
        return appointmentDate >= fromDateStr && appointmentDate <= toDateStr;
      });
    }
...
export const selectFilteredMatches = createSelector(
  [selectAdminMatches, selectAdminMatchFilters],
  (matches, filters) => {
    ...
    if (statusFilter !== undefined && statusFilter !== 'all') {
      const targetStatus = statusFilter;
      filtered = filtered.filter((match) => match.status === targetStatus);
    }
    ...
    if (filters?.fromDate !== undefined && filters.fromDate.length > 0) {
      ...
      filtered = filtered.filter((match) => {
        ...
        return matchDate >= fromDateStr && matchDate <= toDateStr;
      });
    }
...
export const selectFilteredAuditLogs = createSelector(
  ...
  (auditLogs, filters) => {
    ...
    if (filters && typeof filters.user === 'string' && filters.user.trim()) {
...
export const selectUserStatistics = createSelector([selectAdminUsers], (users) => {
...
export const selectAdminSkillStatistics = createSelector([selectAdminSkills], (skills) => {
...
export const selectRecentActivity = createSelector([selectAuditLogs], (auditLogs) => {
...
export const selectPendingReports = createSelector([selectModerationReports], (reports) =>
  reports.filter((report) => report.status === 'pending')
);

export const selectCriticalSystemIssues = createSelector([selectSystemHealth], (systemHealth) => {
  if (systemHealth === undefined) return [];
  ...
  if (systemHealth.infrastructure?.messageQueue.status === 'disconnected')
    issues.push('Message Queue Down');
...
export const selectIsLoadingSecurityAlerts = (state: RootState): boolean =>
  state.admin.isLoadingSecurityAlerts;
export const selectIsLoadingSecurityStatistics = (state: RootState): boolean =>
  state.admin.isLoadingSecurityStatistics;
export const selectSecurityAlertError = (state: RootState): string | undefined =>
  state.admin.securityAlertError;
