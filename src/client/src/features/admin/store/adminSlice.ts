...
import type { AdminUser, AdminSkill, ModerationReport } from '../types/Admin';
...
const adminSlice = createSlice({
  ...
  reducers: {
    ...
    setAppointmentFilters: (state, action: PayloadAction<Partial<AdminAppointmentFilters>>) => {
      if (state.filters) {
        const currentFilters = state.filters.appointments;
        const newFilters: AdminAppointmentFilters = {
          status: action.payload.status ?? currentFilters.status,
          fromDate: action.payload.fromDate ?? currentFilters.fromDate,
          toDate: action.payload.toDate ?? currentFilters.toDate,
          searchTerm: action.payload.searchTerm ?? currentFilters.searchTerm,
        };
        state.filters.appointments = newFilters;
      }
    },
    setMatchFilters: (state, action: PayloadAction<Partial<AdminMatchFilters>>) => {
      if (state.filters) {
        const currentFilters = state.filters.matches;
        const newFilters: AdminMatchFilters = {
          status: action.payload.status ?? currentFilters.status,
          fromDate: action.payload.fromDate ?? currentFilters.fromDate,
          toDate: action.payload.toDate ?? currentFilters.toDate,
          searchTerm: action.payload.searchTerm ?? currentFilters.searchTerm,
        };
        state.filters.matches = newFilters;
      }
    },
    ...
    moderateSkillOptimistic: (
      ...
      if (skill) {
        ...
        skill.status = statusMap[action.payload.action];
      }
    },
    ...
    handleReportOptimistic: (
      ...
      if (report) {
        ...
        report.status = statusMap[action.payload.action];
      }
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAdminDashboard.rejected, (state, action) => {
        state.isLoading = false;
        state.errorMessage =
          action.payload?.message ?? action.error.message ?? 'Failed to load dashboard';
      })
      ...
      .addCase(fetchAdminUsers.rejected, (state, action) => {
        state.isLoadingUsers = false;
        state.userError = action.payload?.message ?? action.error.message ?? 'Failed to load users';
      })
      ...
      .addCase(fetchAdminSkills.rejected, (state, action) => {
        state.isLoadingSkills = false;
        state.skillError =
          action.payload?.message ?? action.error.message ?? 'Failed to load skills';
      })
      ...
      .addCase(fetchAdminAppointments.rejected, (state, action) => {
        ...
        state.appointmentError =
          action.payload?.message ?? action.error.message ?? 'Failed to load appointments';
      })
      ...
      .addCase(fetchAdminMatches.rejected, (state, action) => {
        state.isLoadingMatches = false;
        state.matchError =
          action.payload?.message ?? action.error.message ?? 'Failed to load matches';
      })
      ...
      .addCase(fetchAdminAnalytics.rejected, (state, action) => {
        ...
        state.analyticsError =
          action.payload?.message ?? action.error.message ?? 'Failed to load analytics';
      })
      ...
      .addCase(fetchSystemHealth.rejected, (state, action) => {
        ...
        state.systemHealthError =
          action.payload?.message ?? action.error.message ?? 'Failed to load system health';
      })
      ...
      .addCase(fetchAuditLogs.rejected, (state, action) => {
        state.isLoadingAuditLogs = false;
        state.auditLogError =
          action.payload?.message ?? action.error.message ?? 'Failed to load audit logs';
      })
      ...
      .addCase(fetchModerationReports.rejected, (state, action) => {
        state.isLoadingReports = false;
        state.reportError =
          action.payload?.message ?? action.error.message ?? 'Failed to load moderation reports';
      })
      ...
      .addCase(fetchAdminSettings.rejected, (state, action) => {
        state.isLoadingSettings = false;
        state.settingsError =
          action.payload?.message ?? action.error.message ?? 'Failed to load admin settings';
      })
      ...
      .addCase(fetchSecurityAlerts.rejected, (state, action) => {
        state.isLoadingSecurityAlerts = false;
        state.securityAlertError =
          action.payload?.message ?? action.error.message ?? 'Failed to load security alerts';
      })
      ...
      .addCase(fetchSecurityAlertStatistics.rejected, (state, action) => {
        state.isLoadingSecurityStatistics = false;
        state.securityAlertError =
          action.payload?.message ?? action.error.message ?? 'Failed to load security statistics';
      });
  },
});
