...
const MatchesPage: React.FC = () => {
  ...
  // PERFORMANCE FIX: Memoize filter arrays to prevent recalculation on every render
  ...
  const cancelledMatches = useMemo(
    () => matches.filter((m) => m.status === 'rejected' || m.status === 'cancelled'),
    [matches]
  );

  // Memoize current matches based on tab selection
  const currentMatches = useMemo((): MatchDisplay[] => {
    switch (tabValue) {
      case 0:
  ...
  }, [tabValue, activeMatches, completedMatches, cancelledMatches]);

  // Memoize pagination
  const { totalPages, paginatedMatches } = useMemo(
    () => ({
      totalPages: Math.ceil(currentMatches.length / pageSize),
      paginatedMatches: currentMatches.slice((page - 1) * pageSize, page * pageSize),
    }),
    [currentMatches, page, pageSize]
  );
  ...
  const getMatchTypeLabel = (match: MatchDisplay): string => {
    ...
    if (match.isMonetary) return `${match.offeredAmount ?? 0} ${match.currency ?? '€'}`;
  ...
  const renderMatchCard = (match: MatchDisplay): React.ReactNode => (
    <Grid sx={{ xs: 12, sm: 6, md: 4 }} key={match.id}>
      <Card
        ...
        onClick={() => navigate(`/matchmaking/${match.id}`)}
      >
