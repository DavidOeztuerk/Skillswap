...
import type { MatchRequestDisplay } from '../types/MatchmakingDisplay';

// ============================================================================
// PERFORMANCE: Extract sx objects as constants to prevent recreation
// ============================================================================

const tabPanelContentSx: SxProps<Theme> = { p: 3 };

const statsGridSx: SxProps<Theme> = { mb: 3 };

const statsCardContentSx: SxProps<Theme> = {
  p: 2,
  textAlign: 'center',
};

const tabsCardSx: SxProps<Theme> = { mb: 3 };
...
const threadHeaderSx: SxProps<Theme> = {
  ...mixins.flexBetween,
  mb: 2,
};

const threadUserSx: SxProps<Theme> = {
  ...mixins.flexCenter,
  flex: 1,
};

const avatarPrimarySx: SxProps<Theme> = {
  mr: 2,
  bgcolor: 'primary.main',
};

const avatarSecondarySx: SxProps<Theme> = {
  mr: 2,
  bgcolor: 'secondary.main',
};

const avatarSuccessSx: SxProps<Theme> = {
  mr: 2,
  bgcolor: 'success.main',
};

const timestampBoxSx: SxProps<Theme> = {
  ...mixins.flexCenter,
  gap: 1,
  mt: 0.5,
};
...
const quickActionsSx: SxProps<Theme> = {
  display: 'flex',
  gap: 1,
  mb: 2,
};

const dividerSx: SxProps<Theme> = { my: 2 };

const historyStackSx: SxProps<Theme> = { mt: 2 };

const requestPaperSx: SxProps<Theme> = { p: 2 };

const requestContentSx: SxProps<Theme> = {
  display: 'flex',
  alignItems: 'center',
  gap: 2,
};

const smallAvatarSx = (statusColor: string): SxProps<Theme> => ({
...
const TabPanel = (props: TabPanelProps): React.JSX.Element => {
  ...
  return (
    <div
      ...
      {...other}
    >
      {value === index && <Box sx={tabPanelContentSx}>{children}</Box>}
    </div>
  );
};
...
const MatchmakingPage: React.FC = () => {
  ...
  const initialTab = tabParam ? Number.parseInt(tabParam, 10) : 0;
  ...
  useEffect(() => {
    const newTab = tabParam ? Number.parseInt(tabParam, 10) : 0;
    if (newTab === currentTab || (newTab !== 0 && newTab !== 1)) return;
  ...
  return (
    <PageContainer>
      ...
      {/* Statistics Cards */}
      <Grid container spacing={2} sx={statsGridSx}>
        <Grid size={gridConfigs.stats}>
          <Card>
            <CardContent sx={statsCardContentSx}>
        ...
        <Box>
          {incomingThreads.length === 0 ? (
            <Paper sx={emptyStateSx}>
              <PersonAddIcon sx={emptyStateIconSx} />
            ...
            <Stack spacing={2}>
              {incomingThreads.map((thread) => (
                <Card key={thread.threadId} variant="outlined">
                  <CardContent>
                    {/* Thread Header */}
                    <Box sx={threadHeaderSx}>
                      <Box sx={threadUserSx}>
                        <Avatar sx={avatarPrimarySx}>{thread.otherUser.name[0] || 'U'}</Avatar>
                        <Box flex={1}>
                          <Typography variant="h6">{thread.otherUser.name}</Typography>
                          <Typography variant="body2" color="text.secondary">
                            {thread.skill.name}
                          </Typography>
                          <Box sx={timestampBoxSx}>
                    ...
                    <Collapse in={expandedThreads.has(thread.threadId)}>
                      ...
                      <Stack spacing={2} sx={historyStackSx}>
                        {thread.requests
                          .sort(
                            (a, b) =>
                          ...
                          .map((request) => (
                            <Paper key={request.id} variant="outlined" sx={requestPaperSx}>
                              <Box sx={requestContentSx}>
                                <Avatar sx={smallAvatarSx(getStatusColor(request.status))}>
              ...
              {outgoingThreads.map((thread) => (
                <Card key={thread.threadId} variant="outlined">
                  <CardContent>
                    ...
                    <Box sx={threadHeaderSx}>
                      <Box sx={threadUserSx}>
                        <Avatar sx={avatarSecondarySx}>{thread.otherUser.name[0] || 'U'}</Avatar>
                        <Box flex={1}>
                          <Typography variant="h6">{thread.otherUser.name}</Typography>
                          <Typography variant="body2" color="text.secondary">
                            {thread.skill.name}
