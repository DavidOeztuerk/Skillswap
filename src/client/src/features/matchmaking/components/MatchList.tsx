...
const MatchList: React.FC<MatchListProps> = ({
  ...
  const handleMatchClick = useCallback(
    (matchId: string) => {
      void navigate(`/matchmaking/${matchId}`);
    },
    [navigate]
  );
  ...
  // Pagination - PERFORMANCE FIX: Memoize to prevent recalculation on every render
  const { pageCount, displayedMatches } = useMemo(
    () => ({
      pageCount: Math.ceil(filteredMatches.length / matchesPerPage),
      displayedMatches: filteredMatches.slice(
