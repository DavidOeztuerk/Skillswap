...
import type { UsersEntityState } from './authAdapter';
 ...
 */
export const selectAuthState = (state: RootState): UsersEntityState => state.auth;

/**
 * Select current authenticated user
 */
export const selectAuthUser = (state: RootState): User | undefined => state.auth.user;
...
export const selectHasAnyRole = createSelector(
  ...
  (userRoles, roles): boolean => {
    ...
    return roles.some((role) => normalizedUserRoles.has(role.toLowerCase()));
  }
);
...
export const selectHasAllRoles = createSelector(
  ...
  (userRoles, roles): boolean => {
    ...
    const normalizedUserRoles = new Set(userRoles.map((r) => r.toLowerCase()));
    return roles.every((role) => normalizedUserRoles.has(role.toLowerCase()));
  }
);
