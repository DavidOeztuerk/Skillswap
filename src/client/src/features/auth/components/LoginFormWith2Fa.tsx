...
import { sanitizeInput } from '../../../shared/utils/cryptoHelpers';
import useAuth from '../hooks/useAuth';
import { type LoginFormValues, loginSchema } from '../schemas/auth';
import { login as loginAction } from '../store/authThunks';
...
interface ThunkResult {
  ...
  payload?: ApiResponse<LoginResponse>;
}
...
/**
 * Build login credentials from form data
 */
const buildLoginCredentials = (data: LoginFormValues): LoginRequest => {
...
/**
 * Process login result and determine outcome
 */
type LoginOutcome = 'success' | 'requires2FA' | 'failed';

const processLoginResult = (result: ThunkResult): LoginOutcome => {
...
const LoginFormWith2FA: React.FC<LoginFormWith2FAProps> = ({
  ...
  const onSubmit: SubmitHandler<LoginFormValues> = async (data) => {
    ...
    try {
      ...
      const credentials = buildLoginCredentials(data);
      const result = await withLoading(LOADING_KEY_LOGIN, () => performLogin(credentials));
      const outcome = processLoginResult(result);
  ...
  const hasValidationErrors = Object.keys(errors).some((key) => key !== 'root');
      ...
      <Box sx={{ mt: 3 }}>
        {showForgotPassword ? (
          <>
            ...
            {showRegisterLink ? <Divider sx={{ my: 2 }} /> : null}
          </>
        ) : null}
