...
class StreamManager {
  ...
  registerStream(stream: MediaStream, type: 'camera' | 'screen' | 'remote'): void {
    ...
    console.debug(`📝 StreamManager: Stream stored, total streams now: ${this.streams.size}`);
  ...
  destroyAllStreams(): void {
    console.debug(`🗑️ StreamManager: Destroying all streams (${this.streams.size})`);
  ...
  getDebugInfo(): {
    ...
    const streams = [...this.metadata.values()].map((meta) => {
