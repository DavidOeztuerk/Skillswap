...
export function useSignalRHub(options: UseSignalRHubOptions = {}): UseSignalRHubReturn {
  ...
  const addToQueue = useCallback(
    (
      ...
      logger.log(
        `Message queued: ${method} (${message.id}), queue size: ${String(messageQueueRef.current.length)}`
      );
  ...
  const flushQueue = useCallback(async (): Promise<void> => {
    ...
    logger.group(`Flushing message queue (${String(messageQueueRef.current.length)} messages)`);
    ...
    for (const message of messagesToProcess) {
      try {
        ...
        if (message.retryCount >= message.maxRetries) {
          logger.error(
            `Failed to send queued message after ${String(message.maxRetries)} retries: ${message.id}`,
            error
          );
          removeFromQueue(message.id);
        } else {
          logger.warn(
            `Queued message failed, will retry (${String(message.retryCount)}/${String(message.maxRetries)}): ${message.id}`
          );
        }
      }
    }
    ...
    async <T extends keyof ClientToServerMethods>(
      ...
      for (let attempt = 0; attempt <= retries; attempt++) {
        ...
        try {
          ...
          if (isRetry) {
            logger.log(`${method} succeeded on retry ${String(attempt)}`);
          }
          ...
          if (attempt < retries) {
            ...
            logger.warn(
              `${method} failed (attempt ${String(attempt + 1)}/${String(retries + 1)}), retrying in ${String(delay)}ms...`
            );
  ...
  const startHeartbeat = useCallback(
    (roomId: string) => {
      ...
      logger.log(`Heartbeat started (interval: ${String(heartbeatInterval)}ms)`);
    },
  ...
  const startLatencyMonitoring = useCallback(() => {
    ...
    logger.log(`Latency monitoring started (interval: ${String(latencyCheckInterval)}ms)`);
        ...
        const connection = new HubConnectionBuilder()
          ...
          .withAutomaticReconnect({
            nextRetryDelayInMilliseconds: (retryContext) => {
              ...
              logger.log(
                `Reconnect attempt ${String(retryContext.previousRetryCount + 1)}, delay: ${String(delay)}ms`
              );
              return delay;
            },
          })
  ...
  }, [stopMonitoring, updateConnectionState, logger]);
