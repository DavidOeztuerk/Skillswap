...
export function usePerformance(
  ...
  // Use useState with lazy initialization for stable component ID
  const [componentId] = useState(
    () => `${componentName}-${Math.random().toString(36).substring(2, 11)}`
  );

  // Use useState with lazy initialization for tracker key
  const [trackerKey] = useState(() => `${componentName}[${componentId}]`);
  ...
  useLayoutEffect(() => {
    ...
    const renderTime = startTime !== null ? renderEndTime - startTime : 0;
    ...
    if (import.meta.env.DEV) {
      ...
      if (renderTime > 50) {
        console.warn(
          `🐌 ${componentName}: Slow render ${renderTime.toFixed(2)}ms (render #${String(renderCountRef.current)})`
        );
      }
      ...
      if (renderCountRef.current > 100 && !propsChanged && renderCountRef.current % 50 === 0) {
        console.warn(
          `🔄 ${componentName}: ${String(renderCountRef.current)} renders without prop changes - consider React.memo`
        );
      }
    }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (import.meta.env.DEV && currentRenderCount > 50) {
        console.warn(
          `🧹 ${componentName}: Unmounted after ${String(currentRenderCount)} renders (check for memory leaks)`
        );
      }
    };
  ...
  const startOperation = useCallback((operationName: string, timeoutMs = 30000) => {
    ...
    const timeout = setTimeout(() => {
      if (operations.current.has(operationName)) {
        console.warn(`⏰ Operation timeout: ${operationName} exceeded ${String(timeoutMs)}ms`);
...
export function useMemoryLeakDetection(componentName: string): {
  ...
  // Use useState with lazy initialization to avoid calling Date.now() during render
      ...
      setTimeout(() => {
        ...
        if (import.meta.env.DEV) {
          console.debug('📊 Page Load Performance:', {
            ...
            renderTime: `${String(renderTime)}ms`,
