...
export const selectNotificationsError = (state: RootState) => state.notifications.errorMessage;
export const selectNotificationsConnected = (state: RootState) => state.notifications.isConnected;
export const selectNotificationsConnectionId = (state: RootState) => state.notifications.connectionId;
...
export const selectRecentNotifications = createSelector(
  ...
  (notifications) => {
    ...
    return notifications.filter(notification => 
      new Date(notification.createdAt).getTime() > oneDayAgo
    );
  }
);

export const selectImportantNotifications = createSelector(
  [selectNotifications],
...
export const selectMatchNotifications = createSelector(
  [selectNotifications],
  (notifications) => notifications.filter(notification => 
    notification.type === NotificationType.MatchRequest ||
    notification.type === NotificationType.MatchAccepted ||
...
export const selectNotificationStatistics = createSelector(
  ...
  (all, unread, appointments, matches, system) => ({
    ...
    readPercentage: all.length > 0 
      ? Math.round(((all.length - unread.length) / all.length) * 100) 
      : 0
  })
);
...
export const selectCanReceiveNotificationType = createSelector(
  ...
  (preferences) => (type: string): boolean => {
    switch (type) {
      case 'appointment':
