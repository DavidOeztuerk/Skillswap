 ...
 */
function detectBrowser(ua: string, isIOS: boolean): { name: BrowserInfo['name']; version: string } {
...
function parseUserAgent(): BrowserInfo {
  ...
  const platformFromUA = detectPlatform(ua);
  ...
  const { name, version } = detectBrowser(ua, isIOS);
  ...
  if (info.name === 'Safari') {
    ...
    if (info.majorVersion < 17) {
      return {
        ...
        devices: [],
        canSetSinkId: false,
        fallbackMessage: `Safari ${info.majorVersion} unterstützt keine Lautsprecher-Auswahl. Bitte nutze deinen Standard-Lautsprecher.`,
      };
    }
...
export function getE2EECompatibilityInfo(): {
  ...
  return {
    ...
    browser: info.name,
    version: info.majorVersion,
    message: `❌ E2EE nicht unterstützt in ${info.name} ${info.majorVersion}`,
    recommendation,
