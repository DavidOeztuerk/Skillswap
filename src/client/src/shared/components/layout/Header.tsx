import React, { useState, useCallback, memo, type JSX } from 'react';
...
const Header: React.FC<HeaderProps> = memo(
  ({ drawerWidth, onDrawerToggle, darkMode, onToggleTheme }) => {
    ...
    }, [handleMenuClose, handleMoreMenuClose, logout, navigate]);

    const handleProfile = useCallback(() => {
      handleMenuClose();
      handleMoreMenuClose();
      void navigate('/profile');
    }, [handleMenuClose, handleMoreMenuClose, navigate]);
    ...
    const handleThemeToggleFromMenu = useCallback(() => {
      onToggleTheme();
      handleMoreMenuClose();
    }, [onToggleTheme, handleMoreMenuClose]);

    // Render der rechten Seite der Toolbar (Buttons und Icons)
    const renderRightSideItems = (): JSX.Element => {
      if (isMobile) {
        // Mobile: nur Icons für Notifikationen und Mehr-Menü
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <IconButton color="inherit" onClick={handleOpenMobileSearch} aria-label="Suche öffnen">
              <SearchIcon />
            </IconButton>

            {isAuthenticated ? <NotificationBell /> : null}

            <IconButton color="inherit" aria-label="Mehr Optionen" onClick={handleMoreMenuOpen}>
              <MoreVertIcon />
            </IconButton>
          </Box>
        );
    ...
    const renderUnauthenticatedMenuItems = (): JSX.Element[] => [
      <MenuItem key="login" component={Link} to="/auth/login" onClick={handleMoreMenuClose}>
        Anmelden
      </MenuItem>,
      <MenuItem key="register" component={Link} to="/auth/register" onClick={handleMoreMenuClose}>
        Registrieren
      </MenuItem>,
    ];
      ...
      <>
        ...
        >
          <Toolbar sx={{ px: { xs: 1, sm: 2 } }}>
            ...
            >
              {/* Sidebar toggle - only show on tablet, hide on mobile (we use tabbar) and desktop (sidebar is always visible) */}
              {!isHomePage && !isMobile && (
                <IconButton
                  color="inherit"
                  ...
                  sx={{ mr: 1, display: { xs: 'none', sm: 'flex', md: 'none' } }}
                >
                  <MenuIcon />
                </IconButton>
              )}
        ...
        <MobileSearchBar open={searchOpen} onClose={handleCloseMobileSearch} />
        ...
        >
          <MenuItem onClick={handleNotificationMenuClose}>
            <Typography variant="subtitle2">Neue Match-Anfrage von Thomas</Typography>
          </MenuItem>
          <MenuItem onClick={handleNotificationMenuClose}>
            <Typography variant="subtitle2">Termin bestätigt für morgen</Typography>
          </MenuItem>
          <MenuItem onClick={handleNotificationMenuClose}>
            <Typography variant="subtitle2">Dein Skill wurde bestätigt</Typography>
          </MenuItem>
        </Menu>
