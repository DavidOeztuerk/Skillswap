...
const SearchResultsPage: React.FC = () => {
  ...
  const { userResults, allSkills, isLoading, userPagination, allSkillsPagination } = useAppSelector(
    (state) => state.search
  );
  ...
  const performSearch = useCallback((query: string, type: string, pageNumber = 1) => {
    if (!query.trim()) return;

    if (type === 'users' || type === 'all') {
      // TODO: Implement user search
      console.debug('Searching users:', { pageNumber, pageSize: 10 });
    }

    if (type === 'skills' || type === 'all') {
      // TODO: Implement skill search
      console.debug('Searching skills:', { pageNumber, pageSize: 10 });
    }
  }, []);
