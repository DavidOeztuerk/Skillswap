...
const RescheduleDialog: React.FC<RescheduleDialogProps> = memo(
  ({ open, onClose, appointment, onReschedule, availableSlots = [] }) => {
    ...
    const combineDateTime = useCallback((date: Date, time: Date): Date => {
    ...
    const conflicts = useMemo(() => {
      ...
      return conflictList;
    }, [selectedDate, selectedTime, currentDateTime, availableSlots, combineDateTime]);

    // Format date and time for display
    const formatDateTime = useCallback(
      (date: Date): string => format(date, "EEEE, d. MMMM yyyy 'um' HH:mm 'Uhr'", { locale: de }),
      []
    );
    ...
    const handleConfirm = useCallback(async () => {
      ...
      if (
        conflicts.length > 0 &&
        !window.confirm('Es gibt Konflikte mit diesem Termin. Möchten Sie trotzdem fortfahren?')
      ) {
        return;
      }
            ...
            <Box>
              ...
              {availableSlots.length > 0 && selectedDate ? (
                ...
                <Stack spacing={3}>
                  ...
                  <TextField
                    select
                    label={`Dauer (optional - aktuell: ${String(duration)} Minuten)`}
                    value={newDuration ?? ''}
              ...
              <Stack spacing={2}>
                ...
                <Card sx={{ bgcolor: 'success.lighter' }}>
                  <CardContent>
                    ...
                    {newDuration !== null && (
                      <Typography variant="body2" color="text.secondary">
                        Neue Dauer: {String(newDuration)} Minuten{' '}
                        {newDuration !== duration && `(vorher: ${String(duration)} Minuten)`}
                      </Typography>
                    )}
                  </CardContent>
                </Card>
