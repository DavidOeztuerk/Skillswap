...
const CalendarView: React.FC<CalendarViewProps> = memo(
  ({ appointments, onAppointmentClick, onDateClick }) => {
    ...
    const getAppointmentColor = useCallback((appointment: Appointment): string => {
      if (appointment.status === AppointmentStatus.Cancelled) return '#9e9e9e';
      if (appointment.status === AppointmentStatus.Completed) return '#4caf50';
      if (appointment.isSkillExchange === true) return '#ff9800';
      if (appointment.isMonetary === true) return '#2196f3';
      return '#9c27b0';
    }, []);
      ...
      return (
        <Box>
          ...
          {weeks.map((week, weekIndex) => (
            <Grid container spacing={0} key={String(weekIndex)}>
          ...
          >
            ...
            {(isToday(weekStart) || weekDays.some((day) => isToday(day))) && (
              <Box
                sx={{
                  position: 'absolute',
                  top: `${String(currentTimePositionPx)}px`,
                  left: 0,
                  right: 0,
                  height: 2,
                  bgcolor: 'error.main',
                    ...
                    >
                      {hourAppointments.map((apt) => (
                        <Card
                          ...
                          sx={{
                            position: 'absolute',
                            top: `${String((new Date(apt.scheduledDate).getMinutes() / 60) * 100)}%`,
                            left: 2,
                            right: 2,
                            height: `${String((apt.durationMinutes / 60) * 60)}px`,
                            bgcolor: getAppointmentColor(apt),
                            color: 'white',
                            cursor: 'pointer',
                            zIndex: 5,
                    ...
                    {hourAppointments.map((apt) => {
                      ...
                      return (
                        <Card
                          ...
                          sx={{
                            position: 'absolute',
                            top: `${String((minutes / 60) * 60)}px`,
                            left: 8,
                            right: 8,
                            minHeight: `${String((apt.durationMinutes / 60) * 60)}px`,
                            bgcolor: getAppointmentColor(apt),
                            color: 'white',
                            cursor: 'pointer',
                            zIndex: 5,
                        ...
                        >
                          <CardContent>
                            ...
                            {apt.sessionNumber !== undefined && apt.sessionNumber > 1 && (
                              <Chip
                                label={`Session ${String(apt.sessionNumber)}/${String(apt.totalSessionsInSeries)}`}
                                size="small"
      ...
      >
        <Box sx={{ mb: 2 }}>
          ...
          <Stack direction="row" spacing={1} sx={{ mt: 2 }}>
            ...
            />
            <Chip label="Abgeschlossen" size="small" sx={{ bgcolor: '#4caf50', color: 'white' }} />
            <Chip label="Abgesagt" size="small" sx={{ bgcolor: '#9e9e9e', color: 'white' }} />
            <Box sx={{ flexGrow: 1 }} />
