...
const MeetingLinkSection: React.FC<MeetingLinkSectionProps> = memo(
  ({
    ...
    const [generatedAt, setGeneratedAt] = useState<Date | null>(
      linkGeneratedAt !== undefined && linkGeneratedAt !== null ? new Date(linkGeneratedAt) : null
    );
    ...
    useEffect(() => {
      const timer = setInterval(() => {
        ...
        if (generatedAt && activationDelayMinutes > 0) {
          ...
          const now = new Date();
          const secondsUntilActivation = Math.max(
            0,
            Math.floor((activationTime.getTime() - now.getTime()) / 1000)
          );
    ...
    const meetingStatus = useMemo(() => {
      ...
      if (isBefore(now, earlyJoinTime)) {
        ...
        return {
          canJoin: false,
          status: 'waiting',
          message: `Beitritt in ${String(minutesUntilJoin)} Minuten möglich`,
        };
      }
    ...
    const handleCopyLink = useCallback(async () => {
      if (effectiveMeetingUrl === null || effectiveMeetingUrl === undefined) return;
            ...
            {isOrganizer && onGenerateLink !== undefined && (
              <Box sx={{ textAlign: 'center' }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
                  Generiere einen sicheren Video-Meeting Link für diesen Termin. Der Link wird
                  automatisch an alle Teilnehmer gesendet.
                </Typography>
              </Box>
            )}
          </>
