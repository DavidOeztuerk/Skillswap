...
const SecurityAlertCard: React.FC<SecurityAlertCardProps> = memo(
  ({ alert, onDismiss, onMarkRead, onViewDetails }) => {
    ...
    const getLevelColor = useMemo(
      () =>
        (level: SecurityAlertLevel): 'error' | 'warning' | 'info' | 'success' | 'default' => {
          switch (level) {
          ...
          <Box display="flex" alignItems="flex-start" justifyContent="space-between">
            <Box display="flex" alignItems="flex-start" gap={2} flex={1}>
              ...
              <Box flex={1}>
                <Box display="flex" alignItems="center" gap={1} mb={0.5}>
                  ...
                  {alert.isDismissed && <Chip label="Dismissed" size="small" />}
                </Box>
                ...
                <Box display="flex" alignItems="center" gap={1} mb={1}>
                  ...
                  {alert.occurrenceCount > 1 && (
                    <Chip
                      label={`${String(alert.occurrenceCount)}x`}
                      color="warning"
                      size="small"
                    />
                  )}
            ...
            <Box>
              ...
              <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={handleMenuClose}>
                ...
                {onViewDetails && <MenuItem onClick={handleViewDetails}>Details anzeigen</MenuItem>}
