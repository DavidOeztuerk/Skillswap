...
import { GroupWork as GroupWorkIcon, Refresh as RefreshIcon, Settings as SettingsIcon } from '@mui/icons-material';
 ...
 */
const MatchingErrorFallback: React.FC<{ error: Error; onRetry: () => void }> = ({ error, onRetry }) => {
  const navigate = useNavigate();
  const [isRecalculating, setIsRecalculating] = React.useState(false);

  const isAlgorithmError = error.message?.toLowerCase().includes('algorithm') || 
                          error.message?.toLowerCase().includes('calculation');
  
  const isDataError = error.message?.toLowerCase().includes('insufficient') || 
