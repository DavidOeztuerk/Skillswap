 ...
 * FIX: Chrome 143+ erkennt rtpTransform, muss aber encodedStreams verwenden!
 */
...
export class InsertableStreamsHandler {
  ...
  prepareEncodedStreamsForSender(sender: RTCRtpSender, kind: 'video' | 'audio'): boolean {
    ...
    const trackId = sender.track?.id ?? `sender-${kind}-${String(Date.now())}`;
  ...
  private setupWorkerMessageHandler(worker: Worker, type: 'encryption' | 'decryption'): void {
    ...
    worker.onmessage = (event: MessageEvent<WorkerMessage>): void => {
      ...
      // Safari Worker sendet periodische Stats - diese direkt in unsere Stats übernehmen
      // WICHTIG: Safari's onrtctransform Handler verarbeitet Frames direkt im Worker,
      // daher müssen wir die Stats von dort importieren!
      if (msgType === 'stats' && payload !== undefined) {
        const statsPayload = payload as WorkerStatsPayload;
        // Addiere Safari Worker Stats zu unseren Stats
        // (Safari hat separate Encryption/Decryption Worker, daher addieren)
  ...
  applyEncryptionToSender(sender: RTCRtpSender, kind: 'video' | 'audio'): void {
    ...
    switch (this.e2eeMethod) {
      ...
      default:
        console.warn(
          `⚠️ E2EE method ${this.e2eeMethod} not supported - running without encryption`
        );
  ...
  applyDecryptionToReceiver(receiver: RTCRtpReceiver, kind: 'video' | 'audio'): void {
    const trackId = receiver.track.id || `receiver-${kind}-${String(Date.now())}`;
  ...
  async updateWorkerKeys(keyMaterial: E2EEKeyMaterial): Promise<void> {
    console.debug(`🔄 E2EE: Updating worker keys (generation ${String(keyMaterial.generation)})`);
            ...
            new Promise<void>((resolve, reject) => {
              ...
              const handler = (event: MessageEvent<WorkerMessage>): void => {
                if (event.data.type === 'keyUpdated') {
                  ...
                  worker.removeEventListener('message', handler);
                  console.debug(
                    `✅ E2EE: Worker confirmed key update (gen ${String(keyMaterial.generation)})`
                  );
                  resolve();
                }
              };
