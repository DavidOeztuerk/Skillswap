...
const ERROR_MESSAGE_MAP: Record<string, (details?: string) => ErrorDetails> = {
  ...
  [ERROR_CODES.UNAUTHORIZED]: (details) => ({
    ...
    message: details || 'Du musst angemeldet sein, um diese Aktion durchzuführen. Bitte melde dich an.',
  }),

  [ERROR_CODES.INSUFFICIENT_PERMISSIONS]: (details) => ({
    code: ERROR_CODES.INSUFFICIENT_PERMISSIONS,
    title: 'Keine Berechtigung',
    message: details || 'Du hast keine Berechtigung, diese Aktion durchzuführen. Nur der Empfänger des Termins kann ihn bestätigen.',
  }),
  ...
  [ERROR_CODES.INVALID_CREDENTIALS]: (details) => ({
    ...
    message: details || 'Die eingegebenen Anmeldedaten sind ungültig. Bitte überprüfe deine E-Mail-Adresse und dein Passwort.',
  }),
  ...
  [ERROR_CODES.RESOURCE_NOT_FOUND]: (details) => ({
    ...
    message: details || 'Die angeforderte Ressource wurde nicht gefunden. Sie wurde möglicherweise gelöscht oder verschoben.',
  }),

  [ERROR_CODES.RESOURCE_ALREADY_EXISTS]: (details) => ({
    code: ERROR_CODES.RESOURCE_ALREADY_EXISTS,
    title: 'Bereits vorhanden',
    message: details || 'Diese Ressource existiert bereits. Bitte verwende einen anderen Namen oder ID.',
  }),
  ...
  [ERROR_CODES.BUSINESS_RULE_VIOLATION]: (details) => ({
    ...
    message: details || 'Diese Aktion verletzt eine Geschäftsregel und kann nicht durchgeführt werden.',
  }),
  ...
  [ERROR_CODES.APPOINTMENT_PAST_DATE]: (details) => ({
    ...
    message: details || 'Dieser Termin liegt in der Vergangenheit und kann nicht mehr bearbeitet werden.',
  }),

  [ERROR_CODES.SCHEDULING_CONFLICT]: (details) => ({
    code: ERROR_CODES.SCHEDULING_CONFLICT,
    title: 'Terminkonflikt',
    message: details || 'Es gibt einen Konflikt mit einem anderen Termin. Bitte wähle eine andere Zeit.',
  }),
  ...
  [ERROR_CODES.NETWORK_ERROR]: (details) => ({
    ...
    message: details || 'Es gab ein Problem mit der Netzwerkverbindung. Bitte überprüfe deine Internetverbindung und versuche es erneut.',
  }),

  [ERROR_CODES.SERVER_ERROR]: (details) => ({
    code: ERROR_CODES.SERVER_ERROR,
    title: 'Serverfehler',
    message: details || 'Ein unerwarteter Serverfehler ist aufgetreten. Bitte versuche es später erneut.',
  }),
  ...
  [ERROR_CODES.UNKNOWN]: (details) => ({
    ...
    message: details || 'Ein unbekannter Fehler ist aufgetreten. Bitte versuche es erneut oder kontaktiere den Support.',
  }),
};
...
export const mapErrorToMessage = (errorCode?: string, details?: string): ErrorDetails => {
  ...
  return {
    code: errorCode,
    title: 'Fehler',
    message: details || `Ein Fehler ist aufgetreten (${errorCode}). Bitte versuche es erneut.`,
  };
};
 ...
 * @returns ErrorDetails mit title und message
 */
export const extractErrorFromResponse = (response: any): ErrorDetails => {
  const errorCode = response?.errorCode || response?.error?.code;
  const errorMessages = response?.errors || response?.error?.message;

  // Wenn errors ein Array ist, nehme die erste Nachricht
  const details = Array.isArray(errorMessages) ? errorMessages[0] : errorMessages;
