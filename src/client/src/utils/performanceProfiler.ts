...
export class PerformanceProfiler {
  ...
  public getStats(): RenderStats[] {
    return Array.from(this.renderStats.values()).sort((a, b) => b.renderCount - a.renderCount);
  }
  ...
  public getRealTimeStats(): {
    ...
    return {
      ...
      totalRenders: Array.from(this.renderStats.values()).reduce(
        (sum, s) => sum + s.renderCount,
        0
      ),
      averageRenderTime:
        Array.from(this.renderStats.values()).reduce((sum, s) => sum + s.averageRenderTime, 0) /
